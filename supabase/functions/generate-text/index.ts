  ...
  try {
   ...
   // if (rating) systemPrompt += `\nRATING: ${rating}`;
    ...
    if(category.toLowerCase() == 'jokes'){
      ...
      userPrompt += `. Do not use ${subcategory.toLowerCase()} in the joke itself. Make them substantial and complete thoughts. No headers, numbers, or formatting - just the one-liners.`;
    } 
