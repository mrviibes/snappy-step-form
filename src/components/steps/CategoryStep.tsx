...
export default function CategoryStep({
  ...
  return <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        <Input placeholder="Add topic(s) by pressing , or enter" value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={handleAddTag} className="text-base h-14 text-center placeholder:text-muted-foreground/60" disabled={currentTags.length >= 3} />
        
        {/* Tag Count */}
        <div className="text-sm text-muted-foreground text-center">
          {currentTags.length}/3 tags added
        </div>
