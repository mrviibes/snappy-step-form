...
export default function VisualsStep({
  ...
  return <div className="space-y-6">
      ...
      {/* Selection Summary */}
      {currentSubStep > 1 && <Card className="p-4 bg-accent/20">
          <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-4">
              {hasSelectedStyle && <div className="flex items-center gap-2">
                  <span className="font-medium">Style:</span> 
                  <span className="text-primary">{selectedStyle?.title}</span>
                  {currentSubStep > 2 && <Button variant="ghost" size="sm" onClick={() => setCurrentSubStep(1)} className="h-6 px-2 text-xs">
                      Edit
                    </Button>}
                </div>}
              {hasSelectedDimension && currentSubStep > 2 && <div className="flex items-center gap-2">
                  <span className="font-medium">Dimension:</span> 
                  <span className="text-primary">{data.visuals?.dimension}</span>
                  {currentSubStep > 3 && <Button variant="ghost" size="sm" onClick={() => setCurrentSubStep(2)} className="h-6 px-2 text-xs">
                      Edit
                    </Button>}
                </div>}
            </div>
      ...
      {currentSubStep === 1 && <>
          ...
          {hasSelectedStyle && currentSubStep === 1 && <div className="pt-4 text-center">
      ...
      {currentSubStep === 2 && hasSelectedStyle && <>
          ...
          {hasSelectedDimension && currentSubStep === 2 && <div className="pt-4 flex gap-3">
      ...
      {currentSubStep === 3 && showGenerateButton && !showVisualOptions && !isComplete && <>
          ...
          <div className="pt-4">
            ...
            setCurrentSubStep(4);
          }} disabled={isGeneratingVisuals} className="flex-1 h-12 text-base font-medium">
                {isGeneratingVisuals ? <>
      ...
      {currentSubStep === 4 && (showVisualOptions || isGeneratingVisuals) && !isComplete && <>
          ...
          {generatedVisuals.length > 0 && <div className="pt-4">
              <Button variant="outline" onClick={() => setCurrentSubStep(3)} className="w-full">
